...
internal class JdbcPostgresClient(private val config: RemoteDatabaseConfig) : ReadOnlyDatabaseClient {
    override suspend fun query(sql: String, args: Map<String, Any?>): List<List<Any?>> = withContext(Dispatchers.IO) {
        Class.forName("org.postgresql.Driver")
        val url = "jdbc:postgresql://${config.host}:${config.port}/${config.database}?sslmode=${if (config.tls) "require" else "disable"}&connectTimeout=10&socketTimeout=60&ApplicationName=vrcx-mobile"
