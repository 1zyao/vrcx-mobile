...
internal actual object VrcxCrypto {
    actual fun sha1(input: ByteArray): ByteArray = memScoped {
        val output = ByteArray(CC_SHA1_DIGEST_LENGTH)
        input.usePinned { inputPin ->
            output.usePinned { outputPin ->
                CC_SHA1(inputPin.addressOf(0), input.size.convert(), outputPin.addressOf(0).reinterpret())
            }
        }
        output
    }
    ...
    actual fun sha256(input: ByteArray): ByteArray = memScoped {
        ...
        input.usePinned { inputPin ->
            output.usePinned { outputPin ->
                CC_SHA256(inputPin.addressOf(0), input.size.convert(), outputPin.addressOf(0).reinterpret())
            }
        }
        output
    }
            ...
            value.usePinned { valuePin ->
                output.usePinned { outputPin ->
                    CCHmac(
                        ...
                        valuePin.addressOf(0), value.size.convert(), outputPin.addressOf(0).reinterpret(),
                    )
                }
            }
        }
